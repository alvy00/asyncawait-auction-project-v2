...
const AuctionCard = ({
        ...
        const getHighestBidder = async () => {
            ...
            try {
                const res = await fetch(
                    `/api/admin?target=user_public&id=${auction.highest_bidder_id}`,
                );

                if (!res.ok) return;
                const data = await res.json();
                setWinner(data.name || data.username);
            } catch (err) {
    ...
    const handleBidSubmit = async (
        ...
        try {
            ...
            const res = await fetch(
                `https://asyncawait-auction-project.onrender.com/api/auctions/bid?mode=${mode}`,
                {
                    method: "POST",
                    headers: {
        ...
        >
            ...
            <div className="p-4">
                ...
                <div className="text-sm text-gray-600 flex justify-between items-center mb-3">
                    ...
                    />
                    <span
                        className={`capitalize font-medium ${isEnded ? "text-red-500" : "text-green-600"}`}
                    >
